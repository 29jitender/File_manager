...
public class FileAdapter extends ArrayAdapter<FileObject> {
	...
	public FileAdapter(Activity a, int textViewResourceId,
			ArrayList<FileObject> entries) {
	...
	public View getView(int position, View recycleView, ViewGroup parent) {
		...
		v.setBackgroundColor(mSelectedItemsIds.get(position) ? 0x9934B5E4
				: Color.TRANSPARENT); // changing colr of selected

		final FileObject file = entries.get(position);
		Resources res = v.getResources();

		((ImageView) v.findViewById(R.id.icon))
				.setImageDrawable(getImageForExtension(file.getFile(), res, v));

		((TextView) v.findViewById(R.id.fileName)).setText(file.getName());

		((TextView) v.findViewById(R.id.fileSize)).setText(file.getSize()); // getting
																			// size

		((TextView) v.findViewById(R.id.fileModified)).setText(file
				.getModified()); // last modified

		return v;
	}

	public void sort() {
		Collections.sort(entries, new Comparator<FileObject>() {
			@Override
			public int compare(FileObject p1, FileObject p2) {
				return (p1.getName().toLowerCase(Locale.getDefault()))
		...
		if (file.isDirectory()) {
			...
			if (!extension.equals("")) {
				if (extension.equals(".mp3") // music
						...
						|| extension.equals(".m4a") || extension.equals(".m4p")) {
					...
					d = res.getDrawable(R.drawable.notepad);
						...
						|| extension.equals(".gzip") || extension.equals(".7z")
					...
					if (packageInfo != null) {
						...
						Drawable icon = appInfo.loadIcon(getContext()
								.getPackageManager());
